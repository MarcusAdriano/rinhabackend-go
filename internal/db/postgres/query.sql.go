// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.25.0
// source: query.sql

package postgres

import (
	"context"

	"github.com/google/uuid"
	"github.com/jackc/pgx/v5/pgtype"
)

const countPessoas = `-- name: CountPessoas :one
SELECT COUNT(*)
FROM pessoas
`

func (q *Queries) CountPessoas(ctx context.Context) (int64, error) {
	row := q.db.QueryRow(ctx, countPessoas)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const createPessoa = `-- name: CreatePessoa :exec
INSERT INTO pessoas (id, nome, apelido, stack, nascimento)
VALUES ($1, $2, $3, $4, $5)
`

type CreatePessoaParams struct {
	ID         uuid.UUID
	Nome       string
	Apelido    string
	Stack      pgtype.Text
	Nascimento pgtype.Date
}

func (q *Queries) CreatePessoa(ctx context.Context, arg CreatePessoaParams) error {
	_, err := q.db.Exec(ctx, createPessoa,
		arg.ID,
		arg.Nome,
		arg.Apelido,
		arg.Stack,
		arg.Nascimento,
	)
	return err
}

const getPessoa = `-- name: GetPessoa :one
SELECT id,nome,apelido,stack,nascimento
FROM pessoas
WHERE id = $1 LIMIT 1
`

type GetPessoaRow struct {
	ID         uuid.UUID
	Nome       string
	Apelido    string
	Stack      pgtype.Text
	Nascimento pgtype.Date
}

func (q *Queries) GetPessoa(ctx context.Context, id uuid.UUID) (GetPessoaRow, error) {
	row := q.db.QueryRow(ctx, getPessoa, id)
	var i GetPessoaRow
	err := row.Scan(
		&i.ID,
		&i.Nome,
		&i.Apelido,
		&i.Stack,
		&i.Nascimento,
	)
	return i, err
}

const searchPessoas = `-- name: SearchPessoas :many
SELECT id,nome,apelido,stack,nascimento
FROM pessoas
WHERE busca @@ plainto_tsquery($1)
ORDER BY nascimento DESC
LIMIT 10
`

type SearchPessoasRow struct {
	ID         uuid.UUID
	Nome       string
	Apelido    string
	Stack      pgtype.Text
	Nascimento pgtype.Date
}

func (q *Queries) SearchPessoas(ctx context.Context, plaintoTsquery string) ([]SearchPessoasRow, error) {
	rows, err := q.db.Query(ctx, searchPessoas, plaintoTsquery)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SearchPessoasRow
	for rows.Next() {
		var i SearchPessoasRow
		if err := rows.Scan(
			&i.ID,
			&i.Nome,
			&i.Apelido,
			&i.Stack,
			&i.Nascimento,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
